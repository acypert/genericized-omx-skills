...
- Save extraction and verdict artifacts under `docs/qa/` for web-clone state persistence between iterations.
...
- **After Pass 1 completes**: Write extraction summary to `docs/qa/web-clone-extraction.json` containing:
...
- **After each Pass 4 verification**: Append the composite verdict to `docs/qa/web-clone-verdicts.json`.
- Also persist the `visual` portion of the composite verdict to `docs/qa/visual-verdict.json`, mapping `visual.score` to top-level `score` and `visual.verdict` to top-level `verdict`.
...
- [ ] Visual verdict persisted to `docs/qa/visual-verdict.json`
- [ ] Extraction summary persisted to `docs/qa/web-clone-extraction.json`
</Final_Checklist>
